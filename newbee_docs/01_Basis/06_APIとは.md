...
## 🛠️ コマンド集 (HTTP Methods)

| メソッド | 意味 | SQL対応 |
|---|---|---|
| `GET` | データを取得 | `SELECT` |
| `POST` | 新規データ作成 | `INSERT` |
| `PUT / PATCH` | データ更新 | `UPDATE` |
| `DELETE` | データ削除 | `DELETE` |
...
## 🧠 理解チェック & ミニ課題
- GET/POST/PATCH/DELETE それぞれで「ユーザーのプロフィール」を扱うURLと簡単な本文を紙に書いてみる。
- 401と403の違いを1行で説明できるか試す。
- SupabaseのREST APIリファレンスで、`profiles`テーブルを取得するクエリ例を1つ探す。

## 🧾 用語メモ
- **REST**: HTTPメソッドとリソースを組み合わせて操作を表現する考え方。
- **ステータスコード**: APIが「結果」を返す番号。2xx成功、4xxクライアントエラー、5xxサーバーエラー。
- **クエリパラメータ**: `?limit=10` のようにURL末尾で条件を渡す仕組み。
