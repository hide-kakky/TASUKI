...
| 用語 | 意味 | TASUKIでの使われ方 |
|------|------|-------------------|
| **import (インポート)** | 他のファイルやライブラリを読み込む | 冒頭に書いてある |
| **Class (クラス)** | プログラムの設計図 | `class MyScreen` |
| **Widget (ウィジェット)** | 画面の部品 | `Text`, `Button` など |
| **Provider (プロバイダー)** | データを提供・管理する仕組み | Riverpodで使う |
| **Migration (マイグレーション)** | DBの設計図（履歴） | テーブル作成用のSQL |
| **Seed (シード)** | 初期データ投入 | テストユーザーなどを作るSQL |

## 🛠️ コマンド集 (Reading Tools)

| コマンド | 説明 |
|---|---|
| `Cmd + Click` (Mac) | 関数や変数の定義元にジャンプする（一番使う！） |
| `Cmd + Shift + F` | 全文検索。特定のワードがどこで使われているか探す |
| `Cmd + P` | ファイル名検索。ファイル名がわかっている時に瞬時に開く |
