...
| 用語 | 意味 | 詳細 |
|------|------|-----|
| **BaaS** | Backend as a Service | サーバー構築不要でバックエンド機能を利用できる |
| **Supabase** | オープンソースのBaaS | PostgreSQLベース |
| **RLS** | Row Level Security (行レベルセキュリティ) | 「誰がどのデータを見ていいか」をDBで制御 |
| **Edge Functions** | サーバー側で動く関数 (TypeScript) | 重い処理や秘密の処理を書く |
| **Table Editor** | データをExcelのように管理できる画面 | Supabaseのダッシュボード |
| **SQL Editor** | SQLを直接実行できる画面 | 複雑なクエリやテーブル作成に使う |

## 🛠️ コマンド集 (Supabase CLI)

| コマンド | 説明 |
|---|---|
| `supabase start` | ローカルでSupabaseを起動する (Docker必須) |
| `supabase stop` | ローカルSupabaseを停止する |
| `supabase functions deploy <name>` | Edge Functionを本番にデプロイする |
| `supabase gen types typescript` | DB定義からTypeScriptの型を自動生成する |
| `supabase db diff` | DBの変更差分を確認する |
