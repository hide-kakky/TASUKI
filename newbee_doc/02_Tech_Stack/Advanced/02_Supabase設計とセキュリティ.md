...
| 用語 | 意味 | 関連 |
|------|------|-----|
| **RLS Policy** | アクセス制御ルール | SQLで書く権限設定 |
| **Security Definer** | 特権モード | Linuxの `sudo` みたいなもの |
| **Index (インデックス)** | 検索を高速化する索引 | 本の目次 |
| **Trigger (トリガー)** | データ変更時に自動実行する処理 | `updated_at` 更新など |
| **Stored Procedure** | DBに保存された関数 | 複雑な処理をDB内で完結 |
| **JWT Claims** | トークンに含まれる情報 | `role` や `email` など |
