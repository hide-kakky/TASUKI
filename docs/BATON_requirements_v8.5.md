...
| 変更履歴 | `v8.6` – 既存マニュアル取り込み（Google Docs）と閲覧UIの追加（2025-11-30）<br>`v8.5r1` – 可読性向上と不明点の補足（2025-11-24） |
...
| Phase2（Day4-10） | 7 日 | Flow→Mux→Stock Draft の 1 本線 + Google Docs 取り込み | Frontend + Edge |
| Phase3（Day11-20） | 10 日 | Manager 承認 UI、既存マニュアル追加、一般閲覧 UI、AI 品質計測 | 全員 |

### 3.2 優先度分類
| Priority | 内容 |
| --- | --- |
| **MUST（MVP）** | 長押し録画、オフライン保存→再送、Mux アップロード、Webhook→Edge→Gemini、AI 要約/手順生成、Manager 承認、既存マニュアル取り込み（Google Docs）、一般閲覧 UI |
...
| ペルソナ | 目的 | 主な画面 / アクション |
| --- | --- | --- |
| Staff | 暗黙知の録画・共有。操作は押すだけで終わること。カテゴリ検索から必要なマニュアルを素早く閲覧できる。 | Flow 撮影 UI、オフライン投稿キュー、タイムライン、マニュアル閲覧画面 |
| Manager | AI が生成したマニュアルの品質確認と承認、スタッフ招待。既存マニュアル（Google Docs）取り込み。 | Stock リスト、詳細・承認、メトリクス表示、マニュアル追加モーダル |
...
### 5.4 既存マニュアル取り込み（Google Docs）
1. Manager が Google Docs URL を入力
2. Edge Function `import_google_doc` が本文取得
3. (Optional) Gemini で要約・手順・Tips を生成
4. `manuals` に `source_type='legacy_import'` で draft 保存
5. Manager 承認後 `status='published'`
...
| `manuals` | Stock（AI or 手動） | `id`, `handover_id`, `ai_summary`, `ai_steps`, `ai_tips`, `status`, `approved_by`, `published_at`, `edit_metrics`, `source_type`, `original_doc_url`, `imported_from`, `imported_at` |
...
- `manuals` の INSERT は `manager` 以上のみ許可。
- `manuals` の閲覧は `store_id` が一致する全ユーザーに許可（`status='published'` のみ）。
...
### 7.3 既存マニュアル取り込み（MVP）
- Manager は Google Docs の共有リンクを貼り付けるだけで既存マニュアルを追加できる。
- Gemini により、3行サマリ / ステップ / Tips に整形可能（任意）。
- 取り込み結果は `manuals.status='draft'` として保存され、承認後に一般ユーザーへ公開。

### 7.4 一般ユーザー向け閲覧（MVP）
- **タイムライン表示**：新着 Published マニュアルの表示。
- **カテゴリ検索**：カテゴリごとに Published マニュアルをフィルタ。
- **マニュアル詳細画面**：
  - 要約（3行）
  - 手順
  - Tips
  - 動画（任意）
  - Google Docs 由来も AI 生成も同一 UI で表示。
...
| UX | 一般ユーザーは 30 秒以内に目的マニュアルを見つけられること。閲覧は 1 スクロールで完結すること。 |
| 可用性 | Supabase 99.9%、Mux 99.99%。完全オフライン投稿（再送）をサポート。店舗全員で閲覧しても負荷が増えない構造（キャッシュ前提）。 |
