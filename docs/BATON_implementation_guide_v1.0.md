...
| Phase2 | Day4-10 | Flow→Mux→Stock Draft の 1 本線 + Google Docs 取り込み | 端末 → Mux → Gemini → `manuals.draft` まで E2E 動作。Google Docs 取り込み動作。 |
| Phase3 | Day11-20 | Manager 承認フロー、一般閲覧 UI、負債返済 | Manager UI で承認→公開、一般ユーザー閲覧、manual_edits 記録、Services/Repositories 抽象化 |
...
### 2.4 Google Docs 取り込み（Edge Function）
#### import_google_doc
...
- Google Docs URL を入力し、ドラフトが生成されること。
...
- **既存マニュアル追加モーダル**：
  - タイトル、Google Docs URL、AI 整形 ON/OFF、カテゴリ選択。
  - 「作成」で `import_google_doc` を呼び出し。

### 3.2 一般ユーザー閲覧 UI（MVP）
- **タイムライン（ホーム）**：
  - 新着 Flow と Published マニュアルを混在表示（`updated_at` 降順）。
  - マニュアルカード：サムネ、タイトル、カテゴリ、3行サマリ（短縮）。
- **マニュアル一覧**：
  - カテゴリフィルタ（ホール/キッチン/清掃/安全衛生）。
- **マニュアル詳細**：
  - ワンスクロールで完結。
  - タイトル、カテゴリ、更新日、動画（あれば）、サマリ、手順、Tips。
  - Google Docs 由来も Flow 由来も同一 UI。
...
| 4 | Edge & Gemini：`mux_webhook`, `ai_process_handover`, `import_google_doc`, draft 保存 | Backend |  |
| 5 | Manager フロー：一覧、詳細、承認、Google Docs 追加、ロール判定 | Frontend |  |
| 6 | 一般ユーザー閲覧：タイムライン、カテゴリ検索、詳細表示 | Frontend |  |
| 7 | 負債返済：VideoService/AiService、Repository 化 | 全員 |  |
| 8 | コスト監視 & メトリクス：Cron + Slack、ダッシュボード | Backend |  |
